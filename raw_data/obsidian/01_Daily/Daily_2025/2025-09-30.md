...
Security Handler ( Msg 분리 ) ( 완료 )
IDE TS PCRC Mismatch 수정 건 ( 있는 옵션 roll back 옵션 추가 - Skip )

IDE Issue 분석 ( 완료 )

L0p Issue 분석
 - FPGA Issue ( resovled - fpga 지원 못 함 - RTL 수정 필요 )
 - Skew Issue ( Duplicated Seq - TB 수정 하기로 )

DL Feature Issue ( 일단 완료 )

IDE Test mode Control 추가 ( 수정 진행 중 - 수정 완료 )
Flit Error Logging 추가 ( 옵션 넣어서 해결 - RTL 수정 완료 )

CSR Update ( Security + 추가 하기 - 완료 )

쭉 RTL 검토
SDC ( input, output, test mode, secure control 등 - 완료 )

Cfg Lock 되는 부분 검토하기 ( Tee Limited Stream이 영향 미치는 부분 검토하기 )
lint 살피기
axi slice 살피기
눈 디버깅

...
- 14:20 - 17:00 [[Life Management Tool 개발]] #에너지50 
	- Google Drive 붙이기
	- Google Drive에서 파일 다운로드 받아서 LLM 연결.. ( 이 부분이 잘 안됨 )
	- N8N 강의 다운로더 잘 안됨... 
	- 되게 해서 NotebookLM으로 다운로드 후 조언 받는 중
	- Chat GPT에도 조언 구했지만 잘 안됨..
- 18:30 - 20:30 [[Life Management Tool 개발]] #에너지90 
	- 결국 bug 인듯... javascipt로 filtering 후 LLM 연결하니 잘 됨 ( empty 오류 사라짐 )
	- 구글 드라이브에서 가져와 merge 하는 부분도 java script 활용
	- google drive로 여러 파일 가져오기 완료 ( search + download )
	- loop 돌면서 dataviewjs 없애는 작업 진행

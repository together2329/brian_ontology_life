...
- Firenze PCIe Sub Fail 분석해서 팀장님께 보고 ( 오전 중에 )
- ATS ( ADB 추가 )
- IP Errata Update
- 애매한 부분 확인 필요
	- ltssm sm
	- reserved type ( dll )
	- hal node 에 core register 쪽 error 관련 로직.. 필요한가?
	- core register .. 필요한가??
		- notebook lm 등으로 추가 체크
- rx overflow interrupt 추가
- doe flr, config enable 옵션 넣어서 추가
- guide 에 mse 기반 bar overlap checking 확인 요청 -> SV팀
- doe guide 추가 flr, linkdown 시 OBC handling
- doe 전체 sequence 가이드
- ide
	- rid config ur
	- rid ur completion 
	- ide rid mismatch detection issue ( fm mode ) -> 현우TL님과 확인

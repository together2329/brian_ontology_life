...
- 13:00 - 14:00 [[식사]]
	- 부모님 건강 검진 들어가고~
	- 기다리는 동안 짜장면 + 짬뽕
- 14:00 - 15:45 [[Life Management Tool 개발]] #에너지50 
	- n8n으로 자동화 툴 만들기
		- 1시간 마다 실시간 분석
	- Finance 분석 개선 시도 ( 실패.. )
	- 뭔가 너무 자동화 자체에 신경을 쓰다 보니 머리가 아프다
	- 이걸 진짜 체계화 하려면 많은 지식이 필요할듯
	- 당분간은 그냥 o3 + o4 high mini로 분석하는게 좋을 듯 ( 조직 인증 받아야 o3 사용 가능 + reasoning 도 )
	- 그리고 open ai api는 google drive 및 옵시디언의 자료를 넣어서 분석을 받는 스크립트를 만들어서 활용하는게 좋을듯
	- 그리고 옵시디언에서 연결하기 ( 내가 연결하고 ) - AI에게 도움을 받는 방식으로
	- 당분간 chat gpt + gemini + notebook lm 을 적극적으로 obsidian과 같이 쓰자
	- 기록도 열심히 하고... time viewer는 일단 viewer 용도로만... 
- 16:00 - 17:00 [[일상]]
	- 건강검진 끝나는거 기다리기
- 17:00 - 18:00 [[일상]]
	- 집으로 이동

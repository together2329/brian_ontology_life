...
1. **Flit Error Counter 확장**: 16bit → 32bit 증가 작업
2. **New ECN / Narrow Burst**: 관련 에러 카운터 및 지원 여부 확인
3. **기타 이슈 해결**:
		- **ATC**: rdata (0xFFFF_FFFF or 0x0) 구분 가능성 확인
		- **HLD ATS New Mode**: OHC 제외 모드 검토

FRS TC 확인 ( Firenze-2399 )
 - 재현을 위해 simulation 진행 중 ( 완료 )
 - 분석 진행 예정
...
FBE, LBE case에 대해 종호TL님과 확인 -> HDMA 확인 필요
### Day planner

- 8:30 - 12:30 [[업무]] #에너지80 
	- FRS Debugging ( TB Issue )
	- IDE Test Mode Debugging ( Test Mode 되는 듯 )
		- CSR 만 sync 맞춰서 업데이트 하고, Andrey에게 이야기하기
	- brian coder로 security handler tb 만들어서 실행 성공
	- brian coder로 counter, counter tb 만들어서 vcs sim 성공
	- 이제 어느 정도 agent 처럼 동작함. 진짜 대박임.

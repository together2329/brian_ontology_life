...
- DRS 전에는 Configuration Write -> UR , DRS는 항상 OHC 없음
	- DRS
	- STM INT, LTR, FRS 등
		- 이 부분은 Direct 로 나가야 한다
	- Segment Captured
	- Link Down ISR
		- Inbound Open 때 Segment Captured 확인 후 Setting ( 유지 되나 ? )
	- Segment Captured Interrupt
	- Bus Num Interrupt
	- 그 이후에는 모든 Msg에 OHC 있음 
	- 문제는 없어 보임
- HAL에 IDE Cap Slice 필요
- AIN PCIe Sub Top Synthesis
- AIN Link0 Synthesis
- AIN Link1 Synthesis
- TDISP Handler 관련 Feature
- 다음주에 Firenze Q Test 회의 ( DV 팀 )

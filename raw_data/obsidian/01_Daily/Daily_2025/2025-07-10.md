...
- link wrapper
- irene에게 status 전달
- axi slice debug module 관련
- jay에게 tbd 및 다른 tc status 전달
- 클로드 코드는 100달러 또는 20달러 여러 개로 사용하는 것이 좋을 듯
	- 어차피 최대 한정 있음
	- 또는 api로 쓰는 방법도 고려
	- 얼마가 나가는지 보고 싶음
- 일 잘하는 법 참고
...
- Selective Stream의 Address + RID ( role ) -> 이 부분 명확
	- 주말에 작업
- Header -> 무결성 ( header -> mac 생성 )
- IDE -> data integrity(crc느낌) + encryption(정보 보호) -> 이 부분 명확
- PCRC , ECRC ( 왜 ?? ) 역할
- 64bit pr count rx 왜 ?
- Configuration Lock ( 관련 구현 필요 )
	- detect + lock 기능
	- Ratel CXL IP 참고 필요

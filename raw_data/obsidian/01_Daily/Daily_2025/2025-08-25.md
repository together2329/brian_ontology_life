...
- PCIE SUB TOP ( AXI Slice 넣기 )
- IDE Test Mode 대응
	- Steven 
- Lint Fix ( Error 만 )
- Custom TC 분석
- L0p TC 분석
	- fail 4개 돌려 놓음 ( 승현TL에게 넘김 )
- TDISP PF, VF 관련 로직 확인
    - Config 
    - RH_IDLE -> SEC Register 초기화 넣기
    - ide cap 확인
    - Discard 옵션 ( Link Stream 만 켜져 있을 때 )
    - Config Register 옵션 추가
- Firenze MIF IDE SIM 확인하기 
- FLR 시, 다른 function에 error 생기는 이슈 ( RID Check 로 확인 중 )
- Completion 에 대한 OHC
- 확인 사항 
	- Debug 추가
	- IDE Option 추가
	- IDE TLP는 Msg RX 괜찮나?
	- IDE 만 켰을 때 discard 하는 옵션 ( 이건 추가해두기 - 지금 가능 하니까 )
	- ide TC 확인 필요
- Mem UR Completion Option 추가
- Grizzly Ridge CSR Update
### Day planner

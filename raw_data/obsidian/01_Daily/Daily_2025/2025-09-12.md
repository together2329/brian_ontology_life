...
다음주에 구현하기
OHC parsing 후에 ATS 보내는 부분 구현하기
Parsing 없이 특정 메시지만 보내도록 구현하기
받아서 다시 전송하는 기능 구현하기
다시 전송할때 그냥 전송하는 것과 SRAM 전송하는 것 옵션 만들기 ( 그냥 전송할 때는 FIFO에 넣기 )
DOE FLR Interrupt 확인해보기
DOE 하나 적고 ready 올리는 부분 개성하기 ( lesson learn )
Poisoned TLP roll back 옵션 추가하기
TDISP UR option 추가하기
### Day planner

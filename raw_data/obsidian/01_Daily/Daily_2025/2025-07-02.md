...
* custom tc debugging
	* intx 
	* host rw
	* req stop ( no axi ) -> jira report
* ide tc debugging
	* viktor email
* tdisp 구현 ( 7/15 )
	* msi 관련 별도 메모리 영역 기반 discard ( offset 만 추가하기 - 아주 간단하게 만 )
	* t bit 비교 tdi 마다 되고 안 되고
	* config write -> 모든 register로 변경
* ide option
	* pcrc enable 보고 pcrc overrining 등 옵션
* axi 확인
	* RO관련 VF, PF optino 넣기 ( PF0만 본다 등 )
* timing
	* axi ready
	* axi -> debug 뒤로
	* mux 안으로
* 피렌체 group interrupt register 삭제
* IDE interrupt 연결
### Day planner

### Review

* claude code( 220 ) + alphavantage( 49 ) + google gemini + chat gpt + perplexity
	- 269 ( 36만원 )
	- 경제 공부 한 달 동안 제대로 시작 !! 
	- 나스닥 100 분석 시킴 ( 리포트 뽑기 )
	- 이거 supabase로 저장하고 웹으로 보여주는 기능 좋을 듯 ( AI 분석까지 )
	- 웹에서 AI 분석도 가능한가
* 하루를 꽉차게 잘 살아야 함
	* 아주 반짝거리는 호기심을 기반으로 살아가야 함
	* 잠을 충분히 잘 자야함 ( 8시간 )
	* 행복꾸러미 기반으로 하루를 살아야함 ( 미래를 위해 현재를 희생하면 안 됨 )
* 무한대의 가능성으로 살아야 함
	* 차오르는 기쁨
	* 차오르는 가능성
	* 이어령 선생님 강의 ( 하루를 충만하게 살기 )
* 일찍 일어나서 나오니 살짝 피곤하긴 합니다.
	* 

...
- Grizzly RTL Freeze ( 8월 중순 )
	- Security
		- ecurity handler에 async fifo 넣기 ( 오늘 하기 )
		- IDE Option 모두 추가 ( 오늘 하기 )
			- List Up하고 2개만
	...
	- Timing
	    - core clk mux -> 관련 다시 한 번 더 confirm 받기 ( 완료 )
	        - 피렌체 임플팀 참조
	- 이슈 1개 분석
		- DRS가 깨지는 현상 
			- ide flit mode 1로 한다고 해결 되지 않음
		- ide metadata 부분
			- 이 모듈을 reset 치는 걸로 해보기
			- reset 하니까 해결됨
			- 세부적으로 어디 모듈이 문제인지 확인하기
			- ack 못 받아서 tlp pending 되어 있다가 ( max unack - ack 못 받아서 pending )
			- linkdown 순간 tlp가 dllp insertion logic으로 들어가 버림
			- 이 쓰레기 데이터가 다음 링크 업때 사용
			- Flush 사용 로직 점검하기 ( 이쪽 파악이 필요할 듯 )
	- Debugging
		- Custom TC Debugging ( 일단.. 1차 리포트 )
- AION
	- Shared Sram Logic 변경
		- shared sram 피렌체에서 가져오기 ( 완료 )
		- grizzly ridge arbitration 로직 가져오기 ( 완료 )
		- compile 확인 예정 ( 완료 )
		- pcie_sub compile 추가 확인 중 
			- 이슈 발생해서
	- SRAM GEN ( 가능함 -> 추가 생성 필요 )
		- ucie sram 생성하기 ( 진행 중 )
			- 일단 완성 ( 쪼개어서 생성 )
		- link1 sram 생성하기 ( sram list 는 준비 )
			- 쪼개서 생성중
...
- 6:20 - 11:40 [[업무]] #에너지80 
	- AION HLD
		- shared sram integration 
	- grizzly ridge
		- custom tc debugging
		- link down issue debugging
		- msg handler 업무 이야기
		- clock mux 디자인 사항 전달 

...
tdisp ide fpga 요청
firenze ide mif sim 확인
ide test mode
email check
jenkins status check
다른 tc 들 check ( ide 등 )
invalid tag jira
ide ur completion submit ( option 추가하기 )
tdisp error condition check
config write 개별
flr 개별 동작 확인 ( req stop flr tc)
msg rc flit overflow check 에 sfr 넣기
firenze reset sim 다시 돌리기
### Review

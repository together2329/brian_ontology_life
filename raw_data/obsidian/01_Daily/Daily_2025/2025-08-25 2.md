...
- IDE Test Mode에서 돌아오질 못함 ( counter issue )
	- sim으로 확인 필요 
- Custom TC 분석
	- Fail TC 는 모두 돌려 놓음 -> 분석 후 답변 하기 ( 일단 1차 답변 함 )
	- 나머지는 추가 분석 필요 ( debugging 후 답변 )
- TDISP PF, VF 관련 로직 확인
	- Completion, PNP 의 function id가 VF면 0x80이 추가된 이후가 된다 ( 이거 감안해서 코드 수정 필요 ) ( 완료 )
	- BAR01 조건 추가 ( 완료 )
	- vf, pf 조건 추가  ( 완료 )
	- selective 에서... tlp type 별로 filtering 필요 할 듯 ( 완료 - msg 다 제거 -> 승훈TL님 과 확인 필요)
		- id routed msg, local terminated 등 ( type 별.. )
	- flush는 또 0x80이 감안 안 되어 있음 -> 그냥 128:0 으로 넣어야 함
	- Configuration Reqeuest Logic 확인
	- secure -> insecure.. 
	- ide cap 확인 
- Email Check
- AION Synthesis 대응
	- link0/1 wrapper sdc 릴리즈 함 
	- pcie sub top list 분리 
		- synthesis 되는 것은 확인함
	- lef 교체 ( lib으로 )
		- 교체 후 합성 해보는 중 ( link는 확인 함 )
			- pcie sub top
			- aion hld top
- Irene - PIPE 쪽 slice multi cycle 경로 확인 후 답변
	- 답변 완료
- Irene Scan Clock Mux ( 완료 )
	- 길봉TL님에게 전달
- Abandon 관련 이슈 
	- 지훈TL님에게 전달
- Lint Fix
- Firenze MIF IDE SIM 확인하기 ( sim 도는 중 )
	- 다음주에 성현TL님 오면 같이 보기 
- Completion 에 대한 OHC
- OHC 관련 ( 명재TL님이 해주시기로 함 - 거의 완료 ) 
	- Config E4 
	- Mem , Cfg -> Reg Top
- FLR 시, 다른 function에 error 생기는 이슈 
	- completion 에 옵션 넣는 것 고려 ( 일단 돌려 보는 중 )
- MCTP Guide 추가
	- header 포함
	- OHC-A,C 추가
- Debug 추가

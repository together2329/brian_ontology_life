...
- gen7 synopsys ip 생성
	- 생성 필요함 
- BAR overlap 옵션 다시 필요할지도 
    - bme, mse, lsb, d0
    - Synopsys GEN7 RTL 생성해서 BAR Overlapping 분석하기
...
- MSE 가 필요할지 전TL님과 논의하기
    - 제외하고 그냥 debugging 추가하기 
        - 모든 ur 조건 ( RTL 완료 )
        - Lint 돌리고 추가해놓기
- shared sram - correctable error, uncorrectable error counter 연결하기 ( 완료 )
...
- gen7 synopsys ip 생성
	- 생성 필요함 
- Auto Reset의 sync ff -> rst_sync 로 변경하기 ( 완료 )
    - glitch free async reset 인지 검토 필요
- KAT TEST Issue Follow Up ( 동작 확인 완료 )
    - 현우tl님 질문 
- msg rx debugging 추가하기
    - main state
    - debug reg 24
    - drop counter 비어 있는 곳 있음
    - 왜 안 보이지... axi는 왔는데 drop이 없음..
- msg csr 추가 ( header 쪽 )

---

- HLD RTL 검토 (Firenze 대비)
- 전반적으로 놓친 부분 없는지 검토

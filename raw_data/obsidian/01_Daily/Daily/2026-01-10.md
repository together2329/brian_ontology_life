...
- DRS 두번 쏘는 부분
- RM이 64B unlign burst 보내는 부분
- HLD Lint ( Msg Handler )
- DV Progress ( Fail TC 분석 )
- Firenze Lint
- L1 debugging
- 수정한 부분 체크, 문제없나 계속 체크
### Day planner

- 6:30 - 10:30 [[업무]] #에너지100 
	- 집중해서 진행
	- ide lint 다시 한 번 체크 ( overflow issue - 발생할 가능성 거의 없음 )
	- fail tc 분석 및 리포트
		- sfr setting ( csr mismatch, rtl issue )
		- inbound burst ( pass )
		- pack pressure ( credit issue )
		- ide ( cfg req -> cpl ), ide cpl is wrong expectation 
		- l1 sub pm reg access ( l1.2를 기다리는데 l1.1 만 enable )
		- ip sram ecc ( ur 무시 가능 )
	- rm 64B 한 번 agent로 시도 -> llm 좋은 모델로 다시 해볼 필요 
	- back pressure 한 번 agent로 시도 -> llm 좋은 모델로 다시 해볼 필요

...
- Career
	- MCTP Dis-Assemble 답변 주기 
		- 1차로 status 요청
	- DOE delay reset 추가 ( 추가 필요 )
		- flr delay 
		- config enable delay
		- guide 에 명시 ( linkdown, flr, perst 시 해야할 일들 )
	- shift logic lint 확인 필요 ( 추가 필요 )
		- + , << 연산 모두 수정 후 성아TL님과 리뷰 
	- Synopsys GEN7 IP 생성 ( Configuration Cadence 랑 맞추기 )
	- selective stream not permitted 확인 필요 ( 확인 완료 - TB Issue )
		- 승훈TL님에게 요청
			- insecure 일 때 나가면 안 됨?
			- config write type 0 는 selective 받으면 안 됨?
			- Type0 IDE TLP 전송하면 ? 
			- Type1 IDE TLP 전송하면 ? 
	- RID 확인 필요 ( 추가 필요 )
		- unexptected completion
		- unsupported request ( config req )
		- rid mismatch detection ( ide fm nfm 
	- l0p sim fail 이유 분석 필요 ( 1차로 확인 완료 )
	- UR counter debugging 추가 ( 확인 완료 )
	- MSE guide 추가 ( bar overlap 방지) ( 확인 완료 )
- Finance
	- 소수몽키 유튜브 정리하기
	- 윌리엄 글 정리하기
	- 메모리 투자
...
- 8:00 - 8:30 [[일상]] #에너지80 
	- 나갈 준비 하기 ( 수분 크림, 재생 크림, 썬크림, 썬크림 톤업 )
	- 푹 자서 충분한 수면을 취했습니다.
- 8:30 - 9:20 [[경제 공부]] #에너지90 
	- 소수 몽키 유튜브 듣기 ( 3번 반복 )
	- 3번 반복하면서 듣는게 좋습니다. 다시 한 번 곱씹고 생각할 여유가 생깁니다.
- 9:20 - 10:10 [[휴식 & 쉼호흡]] #에너지80 
	- 다 같이 오리지널에서 커피 한잔
	- 니로 엔진 클러치에 대한 공부
- 10:10 - 10:34 [[업무]] #에너지80 
	- 업무 리스트 정리
	- fail tc 분석 및 정리
	- l0p debugging ( 옵션이 clear 됨 )
	- msg receiver << 연산 리뷰

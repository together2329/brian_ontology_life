...
- HLD, CLD UCIe Link Up 
	- reset 중복 이슈 -> 제거 후 동작 확인
- UCIe Version Sync Up 진행 ( HLD, FLD, CLD )
	- UCIe Version 재정의 ( DB 날짜 기준 )
	- HLD <-> CLD는 Sync Up 됨
	- FLD <-> CLD는 Sync Up 되지 않아 Local 수정해서 제공 예정 ( 해당 DB로 ASIC, FPGA E2E 모두 가능 - UCIe IF 관점 )
- HLD Release to ScaleFlux ( 2.1 version 에 Integration 요청 )
	- Interface 변경 없음 ( SMPL Flit Format 6 )
	- SRAM Memory Release
	- STD ( Scale Flux에 있는걸로 써야 함 )
		- LTV, SVT 다시 한 번 확인 필요 ( 임민수 TL님이 임시로 한 부분이 있는지 )
	- DFT Update
		- DFT Connection 자료 전달 ( DFT Engineer와 직접 소통할 수 있는 창구 요청함 )
	- SRAM Wrapper Update
		- 왜 업데이트 필요한지 확인 요청 ( Jinfu )
	- STD, MEM Lib 챙기기
		- 임민수 TL님에게 합성 요청하기
- FPGA Rom Code Follow Up ( Andrey, 노완준 TL님 )
- 

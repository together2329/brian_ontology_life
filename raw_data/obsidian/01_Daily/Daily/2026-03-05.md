...
- Company
    ...
    - GEN7 Link Up 확인 ( 완료 )
        - PHY Link UP 확인
        - DL Link Up 되어야 함 ( 왜 안 되는지 확인 필요 -> 정지형 TL님 )
	- GEN7 SRAM 교체 후 TLP TX, RX 확인 예정
	- GEN7 
		- Milano Option 연결
		- CXL ( PBR ) 관련 로직 제거
		- SRAM Lint ( Bank 늘어난 부분 )
	- MSI Sim 결과 제공 예정 to SFX (완료)
	- Direct TX 방식에서 remaning count가 다른 VDM RX에 영향을 주지 않는지 확인 필요 (영향 없음)
		- 꼼꼼하게 봐주시는 형근TL님께 감사
- Agent
    - open router
    - claude code
...
- 주식 리뷰 ( 기계적으로 매수 후 , 단기채권 기반으로 비율 맞추기 )
	- 공포장이 왔을 때 대응 메뉴얼 만들기
